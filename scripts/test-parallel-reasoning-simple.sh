...
# Simplified test that uses the same DO session throughout
# by not providing a session ID, letting the server create one
...
MCP_SESSION_ID=$(grep -i "mcp-session-id:" /tmp/init_headers.txt | cut -d' ' -f2 | tr -d '\r\n')

if [ -z "$MCP_SESSION_ID" ]; then
  echo "❌ Failed to get MCP session ID from headers"
...
# Step 2: Initialize parallel reasoning session (using same DO)
...
# Step 3: Submit a reasoning plan (using same DO)
...
# Step 4: List plan status (using same DO)
