...
**Version 5.0.3** | **LLM-Centric Parallel Reasoning, Evidence-Backed, Industry-Aware, Production-Ready**
...
### 🆕 Version 5.0.3 - Session Persistence Fix (2025-10-01)

**Critical Bug Fix**:
- ✅ **Fixed session persistence** - Resolved "session not found" error when submitting plans
- ✅ **Nested Map serialization** - Properly serialize/deserialize `plans` and `plan_results` Maps in session state
- ✅ **Durable Object storage** - Correct handling of complex data structures in Cloudflare DO storage
- ✅ All tests passing (162/162), including new serialization test

**Technical Details**:
- Problem: When sessions were persisted to Durable Object storage, nested Maps (`plans`, `plan_results`) were converted to empty objects `{}` during JSON serialization
- Solution: Custom serialization/deserialization that converts Maps to arrays before storage and back to Maps after loading
- Impact: Session state now persists correctly across requests, enabling multi-step parallel reasoning workflows
